...
def filter_conversations_by_sdk(conversations):
    """Filters conversations for the MetaMask SDK area and retrieves full conversation details"""
    filtered_conversations = []
    for conversation in conversations:
        attributes = conversation.get('custom_attributes', {})
        print(f"Custom Attributes: {attributes}")

        # Check if the conversation belongs to "SDK"
        if attributes.get('MetaMask area', '').strip().lower() == 'sdk':
...
def store_conversations_to_csv(conversations, file_path):
    """Stores filtered SDK conversations into a CSV file"""
...
def main_function():
    """Main function to extract, filter, and save SDK conversations"""
    conversations = search_conversations("2025-1-24 00:00", "2025-1-26 16:00")
    if conversations:
        sdk_conversations = filter_conversations_by_sdk(conversations)

        print(f"SDK Conversations: {len(sdk_conversations)}")

        store_conversations_to_csv(sdk_conversations, '1-26-25-combined_sdk_output-1.csv')
    else:
