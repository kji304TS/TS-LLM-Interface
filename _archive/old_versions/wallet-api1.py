...
def store_conversations_to_csv(conversations, file_path):
    """Stores filtered Wallet API conversations into a CSV file"""
...
def main_function():
    """Main function to extract, filter, and save Wallet API conversations"""
    conversations = search_conversations("2025-1-24 00:00", "2025-1-27 16:00")
    if conversations:
        wallet_api_conversations = filter_conversations_by_wallet_api(conversations)

        print(f"Wallet API Conversations: {len(wallet_api_conversations)}")

        store_conversations_to_csv(wallet_api_conversations, '1-26-25-combined_wallet-api_output-1.csv')
    else:
