...
def store_conversations_to_csv(conversations, file_path):
    ...
    with open(file_path, mode='w', newline='', encoding='utf-8') as file:  # Specify UTF-8 encoding
        ...
        for conversation in conversations:
            ...
            # Extract Wallet Issue category
            attributes = conversation.get('custom_attributes', {})
            wallet_issue = attributes.get('Wallet issue', 'N/A')

            # Debugging to ensure data is being extracted
            print(f"Writing conversation: {conversation_id}, Summary: {summary}, Transcript: {transcript}, Wallet Issue: {wallet_issue}")
...
def main_function():
    ...
    if conversations:
        wallet_conversations = filter_conversations_by_product(conversations, 'Wallet')
        print(f"Wallet Conversations: {len(wallet_conversations)}")
        store_conversations_to_csv(wallet_conversations, 'wallet_conversations-23.csv')
    else:
