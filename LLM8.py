...
# --------------------------
# New: KPI Tag/Attribute mapping configuration
# --------------------------
...
# Default tags/attributes to drive KPIs (can be overridden via env)
FLI_TAGS = set(x.lower() for x in _env_list("FLI_TAGS", [
    "fli", "fraud-loss-investigation", "security-investigation", "fraud_loss_investigation"
]))
FLI_ATTR_KEYS = set(x.lower() for x in _env_list("FLI_ATTR_KEYS", [
    "fli", "fraud loss investigation", "fraud_loss_investigation", "security_investigation"
]))

ELEVATION_MANUAL_TAGS = set(x.lower() for x in _env_list("ELEVATION_MANUAL_TAGS", [
    "elevated-manual", "manual-escalation", "tier-2-manual", "manual_elevation"
]))
ELEVATION_AI_TAGS = set(x.lower() for x in _env_list("ELEVATION_AI_TAGS", [
    "elevated-ai", "auto-escalation", "bot-escalation", "ai_elevation"
]))

PARTNER_REFERRAL_TAGS = set(x.lower() for x in _env_list("PARTNER_REFERRAL_TAGS", [
    "investigative-partner-referred", "partner_referred"
]))
PARTNER_ACCEPTED_TAGS = set(x.lower() for x in _env_list("PARTNER_ACCEPTED_TAGS", [
    "investigative-partner-accepted", "partner_accepted"
]))

TRACE_REFERRAL_TAGS = set(x.lower() for x in _env_list("TRACE_REFERRAL_TAGS", [
    "metamask-trace-referred", "trace_referred"
]))
TRACE_ACCEPTED_TAGS = set(x.lower() for x in _env_list("TRACE_ACCEPTED_TAGS", [
    "metamask-trace-accepted", "trace_accepted"
]))

BLOCKAID_FP_TAGS = set(x.lower() for x in _env_list("BLOCKAID_FP_TAGS", [
    "blockaid-false-positive", "blockaid_fp"
]))

SLA_BREACHED_KEYS = set(x.lower() for x in _env_list("SLA_BREACHED_KEYS", [
    "sla_breached", "sla breached", "sla_violation", "sla violated"
]))
SLA_MET_KEYS = set(x.lower() for x in _env_list("SLA_MET_KEYS", [
    "sla_met", "sla met", "sla_achieved", "sla achieved"
]))

def _extract_tag_names(conv: dict) -> List[str]:
    tags: List[str] = []
    try:
        t = conv.get("tags")
        if isinstance(t, dict):
            data = t.get("data") or t.get("tags") or []
            for item in data:
...
def _extract_csat(conv: dict) -> Tuple[Optional[float], Optional[str]]:
    try:
        cr = conv.get("conversation_rating") or {}
        rating = cr.get("rating")
        remark = cr.get("remark")
        if rating is None:
            return None, remark
        try:
            return float(rating), remark
        except Exception:
            return None, remark
    except Exception:
        return None, None

def _attr_truthy(attributes: dict, candidate_keys_lower: Set[str]) -> bool:
    for k, v in (attributes or {}).items():
        try:
            kl = str(k).strip().lower()
            if kl in candidate_keys_lower and str(v).strip() not in ("", "None", "N/A", "nan"):
                s = str(v).strip().lower()
                if s in ("true", "1", "yes", "y", "ok", "done", "open"):
                    return True
                # numeric positive
                try:
                    if float(s) > 0:
                    ...
                    # any non-empty value for these keys counts as truthy
                    return True
        except Exception:
            continue
    return False

def _derive_kpi_flags(attributes: dict, tags: List[str]) -> Dict[str, object]:
    tags_l = set([t.strip().lower() for t in (tags or [])])
    attrs = attributes or {}

    fli_ticket = (len(tags_l & FLI_TAGS) > 0) or _attr_truthy(attrs, FLI_ATTR_KEYS)
    elevated_manual = len(tags_l & ELEVATION_MANUAL_TAGS) > 0 or _attr_truthy(attrs, set(["elevated_manual", "manual_escalation"]))
    elevated_ai = len(tags_l & ELEVATION_AI_TAGS) > 0 or _attr_truthy(attrs, set(["elevated_ai", "ai_escalation", "auto_escalation"]))
    partner_referred = len(tags_l & PARTNER_REFERRAL_TAGS) > 0 or _attr_truthy(attrs, set(["partner_referred"]))
    partner_accepted = len(tags_l & PARTNER_ACCEPTED_TAGS) > 0 or _attr_truthy(attrs, set(["partner_accepted"]))
    trace_referred = len(tags_l & TRACE_REFERRAL_TAGS) > 0 or _attr_truthy(attrs, set(["trace_referred", "metamask_trace_referred"]))
    trace_accepted = len(tags_l & TRACE_ACCEPTED_TAGS) > 0 or _attr_truthy(attrs, set(["trace_accepted", "metamask_trace_accepted"]))
    blockaid_false_positive = len(tags_l & BLOCKAID_FP_TAGS) > 0 or _attr_truthy(attrs, set(["blockaid_false_positive", "blockaid_fp"]))

    sla_breached = _attr_truthy(attrs, SLA_BREACHED_KEYS)
    sla_met = _attr_truthy(attrs, SLA_MET_KEYS)

    elevated_any = elevated_manual or elevated_ai

    return {
        "fli_ticket": fli_ticket,
        "elevated_manual": elevated_manual,
        "elevated_ai": elevated_ai,
        "elevated_any": elevated_any,
        "partner_referred": partner_referred,
        "partner_accepted": partner_accepted,
        "trace_referred": trace_referred,
        "trace_accepted": trace_accepted,
        "blockaid_false_positive": blockaid_false_positive,
    ...
    for idx, conv in enumerate(conversations, start=1):
        ...
        if matched:
            ...
            if full:
                ...
                # New: enrich with tags and CSAT, and derive KPI flags
                tag_names = _extract_tag_names(full)
                if tag_names:
                    full_attrs["tags"] = ", ".join(tag_names)
                csat_rating, csat_remark = _extract_csat(full)
                if csat_rating is not None:
                    full_attrs["csat_rating"] = csat_rating
                if csat_remark:
                    full_attrs["csat_remark"] = csat_remark
                # Derive KPI flags from tags/attributes
                derived = _derive_kpi_flags(full_attrs, tag_names)
...
def _is_truthy(val: object) -> bool:
    ...
    if s in ("true", "t", "yes", "y", "1", "ok", "done", "open"):
        return True
    # Sometimes status words indicate truthy
    if any(x in s for x in ["referred", "accepted", "ai", "auto", "automated", "breach", "breached", "positive"]):
        return True
    # Numeric positive
    try:
        return float(s) > 0
...
def _find_cols(df: pd.DataFrame, include: List[str], exclude: Optional[List[str]] = None) -> List[str]:
    inc = [i.lower() for i in include]
    exc = [e.lower() for e in (exclude or [])]
    cols: List[str] = []
    for c in df.columns:
        cl = c.lower()
        if all(i in cl for i in inc) and not any(e in cl for e in exc):
            cols.append(c)
    return cols

def _count_any_truthy(df: pd.DataFrame, cols: List[str]) -> int:
    if not cols:
        return 0
    sub = df[cols].copy()
    return int(sub.applymap(_is_truthy).any(axis=1).sum())

def _count_value_match(df: pd.DataFrame, cols: List[str], patterns: List[str]) -> int:
    if not cols:
        return 0
    regex = re.compile("|".join(patterns), flags=re.IGNORECASE)
    def _row_match(val: object) -> bool:
        return bool(regex.search(_norm_str(val)))
    sub = df[cols].copy()
    return int(sub.applymap(_row_match).any(axis=1).sum())

def _avg_csat(df: pd.DataFrame) -> Optional[float]:
    cand_cols = _find_cols(df, ["csat_rating"]) + _find_cols(df, ["csat"]) + _find_cols(df, ["satisfaction"]) + _find_cols(df, ["rating"])
    if not cand_cols:
        return None
    values: List[float] = []
    for c in cand_cols:
...
def _csat_pct(df: pd.DataFrame, threshold: float = 4.0) -> Optional[float]:
    cols = _find_cols(df, ["csat_rating"]) or []
    if not cols:
        return None
    ratings: List[float] = []
    for c in cols:
    ...
    if not ratings:
        return None
    satisfied = sum(1 for r in ratings if r >= threshold)
    return (satisfied / len(ratings)) * 100.0

def _sla_breach_rate(df: pd.DataFrame) -> Optional[float]:
    # Prefer explicit column if present
    if "sla_breached" in df.columns:
        vals = df["sla_breached"].tolist()
        if len(vals) == 0:
            return 0.0
        breaches = sum(1 for v in vals if _is_truthy(v))
        return (breaches / len(vals)) * 100.0

    breach_cols = _find_cols(df, ["sla", "breach"]) + _find_cols(df, ["sla", "breached"]) + _find_cols(df, ["sla", "violat"])  # violation
    met_cols = _find_cols(df, ["sla", "met"]) + _find_cols(df, ["sla", "achiev"])  # achieved
    total = len(df)
    if total == 0:
        return 0.0
    breaches = _count_any_truthy(df, breach_cols)
    if breaches == 0 and met_cols:
        # If only SLA Met is available, infer breaches as not met
        met = _count_any_truthy(df, met_cols)
        breaches = max(0, total - met)
    return (breaches / total) * 100.0

def _elevation_breakdown(df: pd.DataFrame, escalations_detected: int) -> Tuple[int, int, int]:
    # Prefer explicit columns
    if "elevated_manual" in df.columns or "elevated_ai" in df.columns:
        elev_manual = int(df["elevated_manual"].apply(_is_truthy).sum()) if "elevated_manual" in df.columns else 0
        elev_ai = int(df["elevated_ai"].apply(_is_truthy).sum()) if "elevated_ai" in df.columns else 0
        total = len(df)
        not_elev = max(0, total - (elev_manual + elev_ai))
        return not_elev, elev_manual, elev_ai

    ai_cols = _find_cols(df, ["elevat"]) + _find_cols(df, ["escalat"]) + _find_cols(df, ["ai"]) + _find_cols(df, ["auto"]) + _find_cols(df, ["bot"]) + _find_cols(df, ["automated"])  # broad net
    manual_cols = _find_cols(df, ["elevat"]) + _find_cols(df, ["escalat"]) + _find_cols(df, ["manu"]) + _find_cols(df, ["agent"]) + _find_cols(df, ["human"]) + _find_cols(df, ["live"])  # manual hints
    ai_count = 0
    manual_count = 0
    total = len(df)
    if ai_cols:
        ai_count = _count_value_match(df, ai_cols, [r"\bai\b", r"auto(mated)?", r"bot"])  # values mention AI/auto/bot
    if manual_cols:
        manual_count = _count_value_match(df, manual_cols, [r"manual", r"agent", r"human", r"live"])
    if ai_count == 0 and manual_count == 0:
        # fallback: use transcript-detected escalations as manual
        manual_count = escalations_detected
    not_elev = max(0, total - (ai_count + manual_count))
    return not_elev, manual_count, ai_count

def _count_fli_tickets(df: pd.DataFrame) -> int:
    # Prefer explicit column
    if "fli_ticket" in df.columns:
        return int(df["fli_ticket"].apply(_is_truthy).sum())
    # Look for FLI-specific columns
    fli_cols = _find_cols(df, ["fli"]) + _find_cols(df, ["fraud", "loss"]) + _find_cols(df, ["fraud", "investig"])
    if fli_cols:
        return _count_any_truthy(df, fli_cols)
    # Fallback: assume none if explicit not present
    return 0

def _count_partner_referrals(df: pd.DataFrame) -> Tuple[int, int]:
    if "partner_referred" in df.columns or "partner_accepted" in df.columns:
        referred = int(df.get("partner_referred", pd.Series([])).apply(_is_truthy).sum()) if "partner_referred" in df.columns else 0
        accepted = int(df.get("partner_accepted", pd.Series([])).apply(_is_truthy).sum()) if "partner_accepted" in df.columns else 0
        return referred, accepted
    referred_cols = _find_cols(df, ["partner"]) + _find_cols(df, ["investig", "partner"]) + _find_cols(df, ["referr"])
    accepted_cols = _find_cols(df, ["partner", "accept"]) + _find_cols(df, ["referr", "accept"]) + _find_cols(df, ["investig", "accept"])  # accepted
    referred = _count_value_match(df, referred_cols, [r"refer(red|ral)?", r"yes", r"true", r"1"]) if referred_cols else 0
    accepted = _count_value_match(df, accepted_cols, [r"accept(ed)?", r"yes", r"true", r"1"]) if accepted_cols else 0
    return referred, accepted

def _count_trace_referrals(df: pd.DataFrame) -> Tuple[int, int]:
    if "trace_referred" in df.columns or "trace_accepted" in df.columns:
        referred = int(df.get("trace_referred", pd.Series([])).apply(_is_truthy).sum()) if "trace_referred" in df.columns else 0
        accepted = int(df.get("trace_accepted", pd.Series([])).apply(_is_truthy).sum()) if "trace_accepted" in df.columns else 0
        return referred, accepted
    referred_cols = _find_cols(df, ["trace"]) + _find_cols(df, ["metamask", "trace"]) + _find_cols(df, ["referr", "trace"])  # referred
    accepted_cols = _find_cols(df, ["trace", "accept"]) + _find_cols(df, ["metamask", "trace", "accept"])  # accepted
    referred = _count_value_match(df, referred_cols, [r"refer(red|ral)?", r"yes", r"true", r"1"]) if referred_cols else 0
    accepted = _count_value_match(df, accepted_cols, [r"accept(ed)?", r"yes", r"true", r"1"]) if accepted_cols else 0
    return referred, accepted

def _count_blockaid_false_positives(df: pd.DataFrame) -> int:
    if "blockaid_false_positive" in df.columns:
        return int(df["blockaid_false_positive"].apply(_is_truthy).sum())
    cols = _find_cols(df, ["blockaid"]) + _find_cols(df, ["blockaid", "false"]) + _find_cols(df, ["blockaid", "fp"]) + _find_cols(df, ["false", "positive"])  # broad net
    if not cols:
        return 0
    # Prefer values that explicitly indicate false positives
    return _count_value_match(df, cols, [r"false\s*positive", r"fp", r"incorrect\s*flag"])

def _compute_security_kpis(df: pd.DataFrame, escalation_count_transcript: int) -> Dict[str, object]:
    total = len(df)
    fli = _count_fli_tickets(df)

    # Elevation breakdown based on explicit flags when present
    not_elev, elev_manual, elev_ai = _elevation_breakdown(df, escalation_count_transcript)

    partner_ref, partner_acc = _count_partner_referrals(df)
    trace_ref, trace_acc = _count_trace_referrals(df)
    blockaid_fp = _count_blockaid_false_positives(df)
    csat_avg = _avg_csat(df)
    csat_pct = _csat_pct(df)
    sla_breach_pct = _sla_breach_rate(df)
    return {
        "total": total,
        "fli": fli,
        "elev_not": not_elev,
        "elev_manual": elev_manual,
        "elev_ai": elev_ai,
        "partner_ref": partner_ref,
        "partner_acc": partner_acc,
        "trace_ref": trace_ref,
        "trace_acc": trace_acc,
        "blockaid_fp": blockaid_fp,
        "csat_avg": csat_avg,
        "csat_pct": csat_pct,
...
def analyze_xlsx_and_generate_insights(
    ...
    # Escalation detection — prefer explicit elevation flags
    if ("elevated_manual" in df.columns) or ("elevated_ai" in df.columns):
        elev_manual_count = int(df.get("elevated_manual", pd.Series([False]*len(df))).apply(_is_truthy).sum())
        elev_ai_count = int(df.get("elevated_ai", pd.Series([False]*len(df))).apply(_is_truthy).sum())
        escalation_count = elev_manual_count + elev_ai_count
    else:
    ...
    if total_area_rows:
        lines.append(f"Conversations Elevated to Technical Support: {int(escalation_count):,}")

    # Security KPI Snapshot (best-effort, non-breaking)
    if meta_mask_area == "Security":
        kpis = _compute_security_kpis(df, int(escalation_count))
        lines.append("")
        lines.append("🧮 KPI Snapshot — Security")
        lines.append(f"FLI Security Tickets: {kpis['fli']:,}")
        lines.append(
            f"Elevation breakdown — Not Elevated: {kpis['elev_not']:,}, Manual: {kpis['elev_manual']:,}, AI: {kpis['elev_ai']:,}"
        )
        lines.append(
            f"Investigative Partner Referrals — Referred: {kpis['partner_ref']:,}, Accepted: {kpis['partner_acc']:,}"
        )
        lines.append(
            f"MetaMask Trace Referrals — Referred: {kpis['trace_ref']:,}, Accepted: {kpis['trace_acc']:,}"
        )
        lines.append(f"Blockaid False Positives: {kpis['blockaid_fp']:,}")
        csat_pct_str = f"{kpis['csat_pct']:.1f}%" if isinstance(kpis.get('csat_pct'), float) and kpis.get('csat_pct') is not None else "N/A"
        lines.append(f"Overall CSAT: {csat_pct_str}")
        sla_str = f"{kpis['sla_breach_pct']:.1f}%" if isinstance(kpis.get('sla_breach_pct'), float) and kpis.get('sla_breach_pct') is not None else "N/A"
        lines.append(f"SLA Breach Rate: {sla_str}")
    ...
    for issue, cnt in issue_iterable:
        ...
        # Generic taxonomy diagnostics for other areas (Swaps, Ramps, Dashboard, Staking, Card, Snaps)
