...
def _avg_csat(df: pd.DataFrame) -> Optional[float]:
    cand_cols = _find_cols(df, ["csat"]) + _find_cols(df, ["satisfaction"]) + _find_cols(df, ["rating"])
    if not cand_cols:
        return None
    values: List[float] = []
    for c in cand_cols:
...
def _count_fli_tickets(df: pd.DataFrame) -> int:
    ...
    # Fallback: assume all Security conversations are FLI tickets
    return len(df)

def _count_partner_referrals(df: pd.DataFrame) -> Tuple[int, int]:
    referred_cols = _find_cols(df, ["partner"]) + _find_cols(df, ["investig", "partner"]) + _find_cols(df, ["referr"])  # referred status
...
def _compute_security_kpis(df: pd.DataFrame, escalation_count: int) -> Dict[str, object]:
    total = len(df)
    fli = _count_fli_tickets(df)
    not_elev, elev_manual, elev_ai = _elevation_breakdown(df, escalation_count)
...
def analyze_xlsx_and_generate_insights(
    ...
    if meta_mask_area == "Security":
        ...
        csat_str = f"{kpis['csat_avg']:.2f}" if isinstance(kpis.get('csat_avg'), float) and kpis.get('csat_avg') is not None else "N/A"
        lines.append(f"Overall CSAT (avg 1–5): {csat_str}")
