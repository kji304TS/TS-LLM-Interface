...
def store_conversations_to_csv(conversations, file_path):
    """Stores filtered Security conversations into a CSV file"""
...
def main_function():
    """Main function to extract, filter, and save Security conversations"""
    conversations = search_conversations("2025-1-20 00:00", "2025-1-21 16:00")
    if conversations:
        security_conversations = filter_conversations_by_security(conversations)

        print(f"Security Conversations: {len(security_conversations)}")

        store_conversations_to_csv(security_conversations, '1-21-25-combined_security_output-1.csv')
    else:
