...
def store_conversations_to_csv(conversations, file_path):
    """Stores filtered Ramps conversations into a CSV file"""
    ...
    with open(file_path, mode='w', newline='', encoding='utf-8') as file:
        ...
        for conversation in conversations:
            ...
            writer.writerow({
                ...
                'Buy issue': buy_issue,
                'Sell issue': sell_issue
            })


def main_function():
    """Main function to extract, filter, and save Ramps conversations"""
    conversations = search_conversations("2024-12-15 00:00", "2024-12-15 9:00")
    if conversations:
        ramps_conversations = filter_conversations_by_ramps(conversations)

        print(f"Ramps Conversations: {len(ramps_conversations)}")

        store_conversations_to_csv(ramps_conversations, '1-21-25-combined_mmramps_output-2.csv')
    else:
