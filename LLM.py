...
PREDEFINED_PROMPTS = {
    "Top Issues": [
        ...
        "What is the most frequent subcategory in the 'Staking issue' column?",
...
# ✅ Filter conversations by MetaMask area
...
def store_conversations_to_csv(conversations, meta_mask_area, week_start_str, week_end_str):
    """Stores conversations in a dynamically named CSV file to prevent overwriting."""
    
    # ✅ Generate filename using the processed week's date range
    file_name = f"{meta_mask_area.lower()}_conversations_{week_start_str}_to_{week_end_str}.csv"
    file_path = os.path.join(OUTPUT_DIR, file_name)  # Create new unique filename
    ...
    return file_path  # ✅ Return the new filename for uploading



def analyze_csv_and_generate_insights(csv_file, meta_mask_area, week_start_str, week_end_str):
    """Analyzes the CSV file, generates structured insights, and ensures predefined prompts are answered with deeper analysis."""

    print(f"📊 Analyzing {csv_file} for {meta_mask_area}...")
    ...
    # ✅ Generate filename based on the processed week's date range
    ...
    analysis_text = []
    analysis_text.append(f"🚀 **Analysis for {meta_mask_area}**\n")
    analysis_text.append("=" * 50)

    # ✅ Ensure keyword extraction always happens
    top_words = pd.Series(dtype="int")
    keyword_contexts = []  # To store keyword-related context
    ...
    if 'summary' in df.columns and not df['summary'].dropna().empty:
        ...
        if not filtered_words.empty:
            ...
            # ✅ Capture surrounding words for deeper context analysis
    ...
    if issue_columns:
        ...
        if df[issue_col].dropna().empty:
            ...
            # ✅ Automated Analysis Paragraph
            analysis_text.append("\n\n🔹 **Automated Analysis:**")
            
            top_issues = df[issue_col].value_counts().head(3).index.tolist()
            top_counts = df[issue_col].value_counts().head(3).tolist()

            analysis_paragraph = f"The most commonly reported issue for {meta_mask_area} is **{top_issues[0]}**, accounting for {top_counts[0]} occurrences ({issue_percentages[top_issues[0]]:.2f}% of all cases). "
            if len(top_issues) > 1:
                analysis_paragraph += f"This is followed by **{top_issues[1]}**, which had {top_counts[1]} reports ({issue_percentages[top_issues[1]]:.2f}%), "
            if len(top_issues) > 2:
                analysis_paragraph += f"and **{top_issues[2]}**, with {top_counts[2]} cases ({issue_percentages[top_issues[2]]:.2f}%). "

            analysis_text.append(analysis_paragraph)
    ...
    if keyword_contexts:
        ...
        for context in keyword_contexts[:5]:  # Show top 5 relevant summary excerpts
    ...
    return insights_file  # ✅ Ensure this function always returns a valid file path
...
def main_function(start_date, end_date, week_start_str, week_end_str):
    """Extracts conversations, analyzes them, and uploads both conversation CSVs and insights files to Google Drive."""
    ...
    processed_files = set()  # Store unique conversation CSVs
    ...
    for area in CATEGORY_HEADERS.keys():
        ...
        if filtered_conversations:
            ...
            # ✅ Generate and save the conversation CSV file
            csv_file = store_conversations_to_csv(filtered_conversations, area, week_start_str, week_end_str)
            processed_files.add(csv_file)  # ✅ Use a set to ensure uniqueness

            # ✅ Generate the Insights file
            insights_file = analyze_csv_and_generate_insights(csv_file, area, week_start_str, week_end_str)
            if insights_file:
    ...
    print("CSV Files:", list(processed_files))
    print("Insights Files:", list(insights_files))

    # ✅ Upload conversation CSVs **only once**
